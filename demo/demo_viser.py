...
def viser_wrapper(
    ...
    # ---- Add a button to recalibrate camera up ----
    gui_recalib_up = server.gui.add_button("Reset Global Up")
    ...
    def _(_) -> None:
        ...
        q = viser_tf.SO3(client.camera.wxyz)

        # Compute camera basis in world coordinates
        cam_up = q.apply(np.array([0, 1, 0]))
        cam_forward = q.apply(np.array([0, 0, -1]))
        cam_right = np.cross(cam_forward, cam_up)
        cam_up = np.cross(cam_forward, cam_right)
        cam_up /= np.linalg.norm(cam_up)

        cam_up_np = np.array(cam_up, dtype=np.float32)

        # Set as new up direction for scene & all clients
        server.scene.up_direction = cam_up_np
        for c in clients.values():
            c.camera.up_direction = cam_up_np

        print(f"✅ Updated global up direction to {cam_up_np.tolist()}")
...
parser.add_argument("--prediction_file", '-pf',type=str, default=None, help="Path to save/load predictions (optional)")
...
def main():
    ...
    if args.prediction_file is not None and os.path.exists(args.prediction_file):
        data_load = np.load(args.prediction_file, allow_pickle=True)
        predictions = {key: data_load[key] for key in data_load.files}
        print(f"Loaded predictions from {args.prediction_file}")
