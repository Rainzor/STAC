...
def img_from_cam(cam_intrinsics, cam_points, distortion_params=None, default=0.0):
    ...
    if distortion_params is not None:
        if not _DISTORTION_AVAILABLE:
            raise ImportError("vggt.dependency.distortion is required for distortion correction but is not installed.")
...
def inv(mat):
    ...
    Args:
        mat: A square matrix as numpy.ndarray or torch.Tensor.

    Returns:
        Inverted matrix with the same type as input.
    """
    if isinstance(mat, torch.Tensor):
        if mat.dtype == torch.bfloat16:
            mat = torch.linalg.inv(mat.to(torch.float32))
        else:
...
def cam_from_img(pred_tracks, intrinsics, extra_params=None):
    ...
    if extra_params is not None:
        if not _DISTORTION_AVAILABLE:
            raise ImportError("vggt.dependency.distortion is required for undistortion but is not installed.")
